...
\end{equation}
where $k$ is the spring constant , $D_1$ is a distance range to receive a force, and $\bm{l}_{ij}$ is the natural length between the membrane molecules $i-j$. However, this natural length measures the distance between the membrane molecules in the initial state. The force  $\bm{F}^a_i$ that a membrane molecule receives from an actin molecule is assumed to be the repulsive force as follows.
\begin{equation}

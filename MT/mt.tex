...
%初期配置の話
Cell membrane molecules were initially placed at regular intervals on the surface of a cylinder.
The equation of motion of the cell membrane molecule is assumed as follows.
At this time, adjacent index information is saved in order to reduce the calculation amount later.
\begin{equation}
m\frac{d^2\bm{x}_i}{dt^2} = \bm{F}^m_i +  \bm{F}^a_i - \eta \frac{d\bm{x}_i}{dt}
\end{equation}
where  $m$ is the mass, $\eta = 8.9\times10^{-6}\si{~kg/s}$ is the viscous coefficient, $\bm{F}^m_i$ is the elastic force received from the membrane molecule, $\bm{F}^m_a$ is the repulsive force received from the actin molecule, and $\bm{x}_i$ is the position vector of the membrane molecule.
It is assumed that the force  $\bm{F}^m_i$ acting between the membrane molecules can be represented by elastic force:
\begin{equation}
\bm{F}^m_i = \sum_{i \in I_j}  -k((\bm{x}_j -\bm{x}_i )-\bm{l}_{ij} )
\end{equation}
where $k$ is the spring constant, and $\bm{l}_{ij}$ is the natural length between the membrane molecules $i-j$. Molecule  $i$ belongs to the set of index $I$ adjacent to molecule $j$.
The natural length measures the distance between membrane molecules in the initial state.
The force  $\bm{F}^a_i$ that a membrane molecule receives from an actin molecule is assumed to be the repulsive force:
\begin{equation}
